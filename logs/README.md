...
| RESULTS_AGENT.md | The clean-room campaign's experiment log — zero-intervention, agent-only, started from the task statement and the official baseline |
| LOG.jsonl | Machine readable log. Intent records are written before training, results after |
| LOG-replay.jsonl | Validation-only reconstruction of the banking trajectory (see `code/replay_verdicts.py`) — independently reproduces the shipped champion using validation data alone |
