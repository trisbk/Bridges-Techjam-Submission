"""Run 38 post-mortem: why a real, within-user, largely novel signal still
cost the model 0.001-0.003 on validation.

Three label-free-on-validation diagnostics (bucket rates are estimated on
TRAIN rows; validation labels are never read). No training happens here —
this is the accounting that the 3-seed runs are then compared against.

  1. fatigue table         : long_view rate by within-visit position, tab=1
  2. absorption            : the same table conditioned on each shipped
                             recency feature, plus the share of sess_pos's
                             rate variance each one absorbs
  3. within-user structure : map every bucket to its train rate, carry the
                             resulting per-row prior onto the validation
                             tab=1 rows, and split its variance into
                             between-user and within-user parts (only the
                             latter is visible to GAUC/nDCG@5); then centre
                             every column within user and regress sess_pos's
                             prior on the champion's features to isolate the
                             novel within-user signal. The identical
                             measurement is run on tab_n against the
                             pre-promotion feature set as a scale reference,
                             because tab_n is the feature that DID win.

Run from code/:  python3 session_depth_diagnostics.py     (~3 min)
"""
import collections
import numpy as np
