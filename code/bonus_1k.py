"""KuaiRand-1K bonus attempt (sandbox only, 31 Aug night).

Transfers the frozen Pure recipe to KuaiRand-1K WITHOUT re-tuning: same
label (long_view), same official split dates, same metric, same features
(kit base + 7 causal sequence features + tab_n), same objective (listwise
InfoNCE K=4), same model class (FM k=16, lr 1e-3, patience 4), same
committee (5 seeds, per-model z-scored mean). Baseline arm: the kit's
pointwise FM on the 5 base fields, same trainer, 3 seeds.

Two engineering adaptations for the 8x larger data, both disclosed:
  * columnar loading (numpy arrays, not per-row dicts) — the features are
    computed by the same definitions in one chronological pass;
  * sparse Adam updates (only embedding rows touched by a batch update;
    the kit's dense update is infeasible at a ~4M-video vocabulary) and
    frequency-capped vocabularies (ids with >=2 train impressions; the
    rest share an UNK slot — the kit does the same for unseen ids).

Selection is validation-only; test is evaluated once per arm at the end.
Run from code/ (needs ../KuaiRand-1K/data, ~1.1GB from the same Zenodo record):  python3 bonus_1k.py
"""
import csv, os, time
import numpy as np
