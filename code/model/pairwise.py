"""P-series: testing pairwise training to better match the ranking metric.

P1 uses BPR to directly learn whether a positive item should rank above a
negative item for the same user. P2 combines BPR with the original pointwise
objective to retain useful calibration while improving within-user ranking.

All pairs come from TRAIN only, sampling is reproducible, and users without
both positive and negative examples are handled safely.
"""
