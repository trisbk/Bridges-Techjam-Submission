"""Pillar 2: exposure-debiased negative sampling, and the bias frontier.

The listwise objective samples a user's negatives uniformly from their
logged impressions — which are themselves the OLD recommender's exposure
choices. The objective therefore partly learns "score above what the old
system over-exposed," an exposure-popularity artifact.

The correction: sample negatives with probability proportional to
(1 / exposure_count)^lambda, where exposure_count is the video's impression
count in the TRAINING window (a train-only statistic; no leakage). lambda=0
is the shipped recipe; larger lambda down-weights over-exposed items as
negatives.

The measurement is the point: for each lambda we score BOTH test sets —
the standard (biased-exposure) log and the random-exposure log (the
dataset's unbiased sample, evaluation-only per our legality analysis).
The trade-off curve between the two as lambda rises is a statement about
KuaiRand ranking that exists in no textbook: how much biased-log score
must be paid for how much unbiased-exposure gain.

Diagnostic models only; the designated submission is untouched.

Run from code/:  python3 debias_frontier.py     (~20 min)
"""
...
def build_all():
    """Rich features for the standard splits AND the random test rows,
    with histories over the full experienced stream (as in unbiased_eval)."""
...
def sample_lists_weighted(pairs_users, neg_w, rng, K):
    """Like listwise.sample_lists, but negatives drawn per-user with
    probability proportional to neg_w (cumulative-sum sampling)."""
    P, N = [], []
