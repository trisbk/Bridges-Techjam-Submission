"""Run 4: testing two ways to push beyond the BPR gains from Run 3.

The L-series tests a listwise/InfoNCE objective, which focuses more on hard
negatives within each user's candidate list. The C-series tests whether
larger k=32 embeddings improve BPR beyond simply adding model capacity.

Negative samples come only from each user's TRAIN rows, and users without
both positive and negative examples are skipped for the listwise loss.
"""
import time
import numpy as np
