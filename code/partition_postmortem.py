"""Run 39 post-mortem: why partition-exposure features failed.

Uses the same train-only diagnostic from Run 38 to measure whether tag_n and
auth_n add genuinely new within-user ranking signal beyond the champion's
existing features. Validation and test labels are never read, and no model is
trained.

The analysis compares the failed partition-count features against tab_n, the
successful reference feature, using within-user variance after controlling for
existing signals. It also measures how much tag_hist already explains tag_n's
information.
"""
import collections
import numpy as np
